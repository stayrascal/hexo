...
- 另一种，每看一个数据就算一下损失函数，然后求梯度更新参数，这个称为随机梯度下降，stochastic gradient descent。这个方法速度比较快，但是收敛性能不太好，效率比较低，可能在最优点附近晃来晃去，hit不到最优点。两次参数的更新也有可能互相抵消掉，造成目标函数震荡的比较剧烈。
- 为了克服两种方法的缺点，现在一般采用的是一种折中手段，mini-batch gradient decent，小批的梯度下降，这种方法把数据分为若干个批，按批来更新参数，这样，一个批中的一组数据共同决定了本次梯度的方向，下降起来就不容易跑偏，减少了随机性。另一方面因为批的样本数与整个数据集相比小了很多，计算量也不是很大。
- 实践：
	* 如果数据集大小比较小，比如小于2000，使用SGD
	* 大于2000的数据集，每次批量的大小为64到512(根据计算机的内存布局和访问，最好使用2的幂次方)
...
### Regularization为什么能避免过拟合
- 从网络结构上来看，如果我们在损失函数中定义一个非常大的lambda，意味着惩罚越大，会使的权重的值接近于0，这就意味着部分神经元会失去活性，整个网络的神经元变少了，参数就是减少了，对应的学习能力就减弱了
- 从激活函数角度来看，比如激活函数选择为tanh(), 如果我们在损失函数中定义一个非常大的lambda，会使的权重的值接近于0，从而线性计算的结果也接近于0，tanh(x)在x接近于0时表现为一个线性函数，由多个线性函数组成的函数依旧是一个线性函数，所以无法拟合那些过于复杂的函数，减少过拟合的情况发生

### Dropout的使用
- 先设置keepprop为1，确认损失函数曲线连续下降
- 再修改keepprop的值，检查其结果
